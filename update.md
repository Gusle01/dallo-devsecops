...
## 2026-06-04 (7차 — 다중 패치가 3안 대신 1안만 나오는 버그 수정)

### 1. '다중 패치' 선택 시 취약점당 수정안이 1개만 생성되는 문제 수정

- 문제: '다중 패치'를 켜도 취약점당 3가지 수정안(최소/권장/구조적)이 아니라 1개(recommended)만 나왔음. 사용자가 비교·선택할 수 없었음.
- 원인: 취약점 1건당 1번의 LLM 호출로 3안을 받아 `### 옵션 N` 헤더로 분리·파싱하는데(`agent/llm_agent.py` `_parse_multi_response`), LLM(claude-sonnet-4-6)이 헤더를 `## 옵션 1`(# 2개)처럼 낼 때가 많음. 파서의 분할 정규식이 `###`(# 3개)만 인식(`r"###\s*옵션\s*\d"`)해 분할에 실패 → "아무것도 못 뽑음" → 단일 수정안 1개로 폴백됐음(그래서 전부 recommended, 설명이 `**설명:**`으로 시작).
- 수정: 분할 정규식을 유연하게 변경 — 줄 시작의 `#` 1~4개 또는 `**`(굵게), `옵션`/`Option`, 숫자를 모두 인식.
  - `re.split(r"(?im)^[ \t]*(?:#{1,4}|\*\*)[ \t]*(?:옵션|option)[ \t]*\d", response)`
- 수정 파일: `agent/llm_agent.py`

### 검증 (7차)

- 실제 LLM 응답(`## 옵션 1/2/3` 형식)으로 파서가 1 → 3개로 정상 추출 확인(minimal/recommended/structural, 각자 다른 코드). `py_compile` 통과.
- 백엔드 전용 변경이라 `--reload`로 자동 반영(프론트는 이미 취약점당 카드를 여러 개 표시). 다중 패치로 다시 분석하면 3안이 표시됨.
