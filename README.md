...
대시보드에서 보안 취약 코드가 입력되고 `redscan`이 실행되면, Dallo는 먼저 정적 분석으로 취약점을 찾은 뒤 LLM에는 **전체 코드가 아니라 취약점 정보와 필요한 문맥**을 전달합니다. 사용자가 입력한 custom instruction은 이 기본 보안 프롬프트 뒤에 추가됩니다.
